...
echo "========================================"
echo "PSP H.264 Annex-B Proxy for Jellyfin"
echo "========================================"
echo "Port d'écoute: $PROXY_PORT"
echo "Jellyfin: $JELLYFIN_HOST:$JELLYFIN_PORT"
echo "Log: $LOG_FILE"
echo "Prêt à recevoir des connexions PSP..."
echo ""
...
# Créer un script temporaire pour socat (évite les problèmes d'échappement)
HANDLER_SCRIPT="/tmp/psp-proxy-handler-$$.sh"
cat > "$HANDLER_SCRIPT" << 'HANDLER_EOF'
#!/bin/bash
# Handler pour chaque requête HTTP
...
# Lire le reste des headers HTTP (jusqu'à ligne vide)
while read -r HEADER; do
    [ -z "$HEADER" ] && break
    HEADER=$(echo "$HEADER" | tr -d '\r')
done

# Si pas d'ID vidéo, erreur 400
if [ -z "$VIDEO_ID" ]; then
    echo "HTTP/1.1 400 Bad Request"
    echo "Content-Type: text/plain"
    echo "Connection: close"
...
# Exporter les variables pour le handler
export JELLYFIN_HOST JELLYFIN_PORT API_KEY LOG_FILE

# Rendre le handler exécutable
chmod +x "$HANDLER_SCRIPT"

echo "Script handler créé: $HANDLER_SCRIPT"

# Boucle principale avec socat
if command -v socat &> /dev/null; then
    echo "Utilisation de socat pour le serveur HTTP..."
    
    # socat avec timeout et fork pour gérer plusieurs connexions
    socat -T 60 TCP-LISTEN:$PROXY_PORT,fork,reuseaddr EXEC:"$HANDLER_SCRIPT"
else
    echo "socat non trouvé, utilisation de netcat (moins robuste)..."
    # Fallback sur nc (netcat)
    while true; do
        "$HANDLER_SCRIPT" | nc -l -p $PROXY_PORT -q 1
        sleep 0.5
    done
fi
